    ...
    def aggregation(biotypes):
        ...
        for biotype in biotypes:
            ...
            biotype_df = pd.read_table(filename, index_col=0).apply(to_percent, axis=1)
            biotype_df.columns = COLUMN_NAMES
            
            # Update max value
            max_value = max(biotype_df.values.max(), max_value)
            df_dict[biotype] = biotype_df.fillna(0)
...
#         'unprocessed_pseudogene',
    ...
    n_columns = res_gmtk.shape[1] * GMTK_FACTOR + res_mix_hm.shape[1] * MIX_FACTOR + len(res_agg_dict) * NUM_COMPONENTS * AGG_FACTOR
    ...
    f, (ax_gmtk, ax_mix, ax_agg) =\
        plt.subplots(1, 3, figsize=(n_columns, n_rows), sharey=True, \
                     gridspec_kw={"wspace": 0.08, "width_ratios": [res_gmtk.shape[1] * GMTK_FACTOR, res_mix_hm.shape[1] * MIX_FACTOR, len(res_agg_dict) * NUM_COMPONENTS * AGG_FACTOR]})
    ...
    for i in range(1, len(res_agg_dict)):
        ...
        sns.heatmap(res_agg_dict[BIOTYPES[i]], annot=True, cbar=False, vmin=0, vmax=agg_vmax, cmap='Blues', ax=ax_agg_aux, fmt='.5g')
        ax_agg_aux.set_title(BIOTYPES[i], **title_args)
    ...
    g_agg = sns.heatmap(res_agg_dict[BIOTYPES[0]], annot=True, cbar=True, vmin=0, vmax=agg_vmax, cbar_ax=ax_agg_cbar, cmap='Blues', ax=ax_agg, fmt='.5g')
