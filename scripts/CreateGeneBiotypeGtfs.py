...
GTF_GENE_FEATURE_INDEX = 2 #Position of the "feature" parameter in an inerval (2 in GTF format) 

gtf = BedTool(snakemake.input[0])
...
for interval in gtf:
    fields = interval.fields # This is necessary because the following line changes the order of attributes, and this can lead to segtools crashing
...
#Create the files with only the intervals that are genes from each biotype, and add their sizes to the results log dictionary
...
log_file = open('logs/gene_biotype_results', 'w')
